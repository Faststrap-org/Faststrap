...
from faststrap import (
    add_bootstrap, Container, Row, Col, Card, Button, Input, Select,
    Badge, Alert, Modal, Navbar, Table, Tabs, Icon, Dropdown,
    create_theme, ButtonGroup, Toast
)
...
PRODUCTS = {
    "1": {
        "id": "1", "name": "Classic White T-Shirt", "price": 29.99,
        "category": "Clothing", "sizes": ["S", "M", "L", "XL"],
        "stock": 50, "image": "👕", "description": "Premium cotton t-shirt"
    },
    "2": {
        "id": "2", "name": "Denim Jeans", "price": 79.99,
        "category": "Clothing", "sizes": ["28", "30", "32", "34", "36"],
        "stock": 30, "image": "👖", "description": "Classic fit denim jeans"
    },
    "3": {
        "id": "3", "name": "Running Shoes", "price": 89.99,
        "category": "Footwear", "sizes": ["7", "8", "9", "10", "11"],
        "stock": 25, "image": "👟", "description": "Lightweight running shoes"
    },
    "4": {
        "id": "4", "name": "Leather Wallet", "price": 39.99,
        "category": "Accessories", "sizes": ["One Size"],
        "stock": 100, "image": "👛", "description": "Genuine leather wallet"
    },
    "5": {
        "id": "5", "name": "Baseball Cap", "price": 24.99,
        "category": "Accessories", "sizes": ["One Size"],
        "stock": 75, "image": "🧢", "description": "Adjustable baseball cap"
    },
    "6": {
        "id": "6", "name": "Backpack", "price": 59.99,
        "category": "Accessories", "sizes": ["One Size"],
        "stock": 40, "image": "🎒", "description": "Durable travel backpack"
    },
    "7": {
        "id": "7", "name": "Sneakers", "price": 69.99,
        "category": "Footwear", "sizes": ["7", "8", "9", "10", "11"],
        "stock": 35, "image": "👞", "description": "Casual everyday sneakers"
    },
    "8": {
        "id": "8", "name": "Hoodie", "price": 49.99,
        "category": "Clothing", "sizes": ["S", "M", "L", "XL", "XXL"],
        "stock": 45, "image": "🧥", "description": "Cozy pullover hoodie"
    }
}
    ...
    return Html(
        ...
        Body(
            ...
            Navbar(
                ...
                Div(
                    Ul(
                        Li(A("Shop", href="/", cls=f"nav-link {'active' if active_tab=='shop' else ''}"), cls="nav-item"),
                        Li(A("Orders", href="/orders", cls=f"nav-link {'active' if active_tab=='orders' else ''}"), cls="nav-item"),
                        Li(A("Admin", href="/admin", cls=f"nav-link {'active' if active_tab=='admin' else ''}"), cls="nav-item"),
            ...
            Div(
                Container(
                    Row(
                        Col(
                            P("© 2024 FastShop. Built with FastHTML + Faststrap", 
...
def shop(category: str = None, search: str = None):
        ...
        for product in products:
            product_cards.append(
                Col(
                    Card(
                        ...
                        Div(
                            ...
                            Div(
                                ...
                                " ",
                                Badge(f"{product['stock']} in stock", 
                    ...
                    cols=12, sm=6, md=4, lg=3, cls="mb-4"
                )
            )
    ...
    content = Div(
        H1("Welcome to FastShop", cls="mb-4"),
        filter_section,
        products_section
    )
                    ...
                    Col(
                        Card(
                            ...
                            Div(
                                ...
                                Div(
                                    Span("Shipping:"),
                                    Strong("FREE", cls="float-end text-success")
                                ),
                                Hr(),
                                Div(
                                    H5("Total:"),
...
def cart_content():
    ...
    for item_id, item in CART.items():
        cart_rows.append(
            Tr(
                Td(Div(Span(item['image'], style={"font-size": "2rem"}), " ", Strong(item['name']))),
                Td(f"${item['price']:.2f}"),
                Td(f"{item['size']}"),
                Td(
                    ButtonGroup(
                        Button("-", variant="outline-secondary", size="sm",
                              hx_post=f"/cart/decrease/{item_id}",
                              hx_target="#cart-content", hx_swap="innerHTML"),
                        Span(str(item['quantity']), cls="px-3 py-1 bg-light border"),
                        Button("+", variant="outline-secondary", size="sm",
                              hx_post=f"/cart/increase/{item_id}",
                              hx_target="#cart-content", hx_swap="innerHTML")
                ...
                Td(Strong(f"${item['price'] * item['quantity']:.2f}")),
                Td(Button(Icon("trash"), variant="danger", size="sm",
                         hx_post=f"/cart/remove/{item_id}",
                         hx_target="#cart-content", hx_swap="innerHTML",
    ...
    return Div(
        H1("Shopping Cart", cls="mb-4"),
        Table(
            Thead(Tr(Th("Product"), Th("Price"), Th("Size"), Th("Quantity"), Th("Subtotal"), Th("Action"))),
            Tbody(*cart_rows),
            striped=True, hover=True, cls="mb-4"
        ),
        Row(
            Col(A(Button("Continue Shopping", variant="outline-secondary"), href="/"), cols=12, md=6),
            Col(
                Card(
                    H4("Order Summary"), Hr(),
                    Div(
                    ...
                    A(Button("Proceed to Checkout", variant="primary", cls="w-100 mt-3"), href="/checkout"),
                    cls="bg-light"
                ),
                cols=12, md=6
            )
...
def checkout_page():
    ...
    if not CART:
        return page_layout(
            Alert("Your cart is empty!", variant="warning", cls="text-center"),
            "shop"
        )
        ...
        Row(
            Col(
                Card(
                    ...
                    Form(
                        Row(
                            Col(Input("first_name", label="First Name", required=True), cols=12, md=6),
                            Col(Input("last_name", label="Last Name", required=True), cols=12, md=6)
                        ),
            ...
            Col(
                Card(
                    ...
                    Ul(
                        *[Li(f"{item['name']} ({item['size']}) x {item['quantity']} - ${item['price'] * item['quantity']:.2f}")
...
def submit_order(first_name: str, last_name: str, email: str, phone: str,
...
def orders_list():
    ...
    if not ORDERS:
        ...
        for order in sorted(ORDERS.values(), key=lambda x: x['date'], reverse=True):
            ...
            order_rows.append(
                Tr(
                    ...
                    Td(A(Button("View", variant="outline-primary", size="sm"), href=f"/orders/{order['id']}"))
                )
            )
        ...
        content = Div(
            ...
            Table(
                Thead(Tr(Th("Order ID"), Th("Date"), Th("Customer"), Th("Total"), Th("Status"), Th("Action"))),
                Tbody(*order_rows),
                striped=True,
                hover=True
            )
...
def admin_panel():
    ...
    content = Div(
        ...
        Card(
            ...
            Table(
                Thead(Tr(Th("Order ID"), Th("Customer"), Th("Total"), Th("Status"), Th("Date"))),
                Tbody(*[
                    Tr(
                        Td(order['id']),
                        Td(order['customer']['name']),
                        ...
                        Td(order['date'])
                    ) for order in sorted(ORDERS.values(), key=lambda x: x['date'], reverse=True)[:5]
                ]),
                striped=True,
                hover=True
            ) if ORDERS else P("No orders yet", cls="card-body text-muted")
        )
    )
