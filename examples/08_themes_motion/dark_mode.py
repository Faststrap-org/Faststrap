...
add_bootstrap(app, theme="indigo-night", mode=current_theme["mode"], use_cdn=True)
...
def toggle_theme():
    ...
    return NotStr(f"""
        <script>
            document.documentElement.setAttribute('data-bs-theme', '{current_theme["mode"]}');
        </script>
        <span id="theme-status" class="badge bg-success">
            Theme changed to {current_theme["mode"].title()} mode!
        </span>
    """)
    ...
    return Div(
        ...
        Navbar(
            ...
            Div(
                ...
                Button(
                    Icon("moon-stars-fill" if current_theme["mode"] == "dark" else "sun-fill"),
                    " Theme",
